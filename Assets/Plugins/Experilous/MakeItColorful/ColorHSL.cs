...
namespace Experilous.MakeItColorful
{
	...
	/// </summary>
	[Serializable] public struct ColorHSL
	{
		...
		public static ColorHSL FromRGB(float r, float g, float b, float a)
		{
			...
			if (c > 0f)
			{
				if (r == max)
				{
					hsl.h = Mathf.Repeat((g - b) / c, 6f) / 6f;
				}
				else if (g == max)
				{
					hsl.h = ((b - r) / c + 2f) / 6f;
				...
				hsl.s = c / (1f - Mathf.Abs(max + min - 1f));
			}
			else
			{
				hsl.h = 0f;
		...
		public static explicit operator Color(ColorHSL hsl)
		{
			...
			Color rgb = new Color(min, min, min, hsl.a);
			if (c > 0f)
			{
				float scaledHue = hsl.h * 6f;
				if (scaledHue < 1f)
				{
					rgb.r += c;
					rgb.g += c * scaledHue;
		...
		public static ColorHSL FromCMY(float c, float m, float y, float a)
		{
			...
			float avgInverse = 1f - (max + min) * 0.5f;

			if (chroma > 0f)
			{
				if (c == min)
				{
					hsl.h = Mathf.Repeat((y - m) / chroma, 6f) / 6f;
				}
				else if (m == min)
				{
					hsl.h = ((c - y) / chroma + 2f) / 6f;
				}
				else
				{
					hsl.h = ((m - c) / chroma + 4f) / 6f;
				}

				hsl.s = chroma / (1f - Mathf.Abs(avgInverse * 2f - 1f));
			}
			else
			{
				hsl.h = 0f;
		...
		public static ColorHSL FromCMYK(float c, float m, float y, float k, float a)
		{
			...
			float chroma = max - min;
			float avgInverse = kInv - (max + min) * kInv * 0.5f;
			...
			if (chroma > 0f)
			{
				...
				hsl.s = chroma * kInv / (1f - Mathf.Abs(avgInverse * 2f - 1f));
			}
			else
			{
				hsl.h = 0f;
